...
void parser_ObjNew(std::string Token[2048], CLASS_TOKEN *o_tokens){
  ...
  // Add new Obj
  // OBJ_IS_SET[std::make_pair(TheNamespace + TheFunction, Token[3])] = true;
  // OBJ_AT_LINE[std::make_pair(TheNamespace + TheFunction, Token[3])] =
  // IntToString(o_tokens->Line); OBJ_CLASS[std::make_pair(TheNamespace +
  ...
  else if (IsInsideClass)
  {
    if (IsInsideFunction)
    {
      ...
      if (DEBUG)
        DEBUG_MESSAGE("[NEW-LOCAL-CLASS-OBJ] [Generated_ID ' " + Token[2] +
                          " '] = [CLASS ' " + Token[4] + " '] (",
                      o_tokens); // DEBUG
