...
void parser_TwoPointOperator(std::string Token[2048], CLASS_TOKEN *o_tokens){
		...
		)
	{
		...
		if (L_FUN_IS_SET[std::make_pair(Token[1], Token[3])] && Token[4] != "(")
			ErrorCode("من اجل نداء الدالة ' " + Token[3] + " ' يجب اضافه '()' بعد ' " + Token[1] + " " + Token[2] + " " + Token[3] + " ' ", o_tokens);
		...
		if (Token[4] == "(")
		{
			...
			if (L_FUN_IS_SET[std::make_pair(Token[1], Token[3])])
			{
				...
				if(DEBUG)DEBUG_MESSAGE("[NS'" + Token[1] + " ':LOCAL_FUNC'" + Token[3] + " ' ( ", o_tokens); // DEBUG
				...
				TempTokenCount = 0;
				for (int p = 5; p <= o_tokens->TOTAL[o_tokens->Line]; p++) // | a, b + 2, c) |
				{
					if (Token[p] != "")
					{
				...
				ScriptSyntaxBuffer = CHECK_CALL_FUN_ARG(false, Token[1], Token[3], 0, TheNamespace, TheFunction, TempToken, (TempTokenCount - 1),  o_tokens);
				...
				// *** Generate Code ***
				ScriptBuffer = " NS_" + ID[Token[1]] + "::FUNCTION_" + ID[Token[3]] + "(" + ScriptSyntaxBuffer + "); \n";
			...
			// ---------------------------------------------------------------------------------
				...
				ErrorCode("المجال ' " + Token[1] + " ' لا تحتوي على دالة محليه بإسم ' " + Token[3] + " ' ", o_tokens);
	...
	{
		// Exception !
		ErrorCode("أمر غير معروف ' " + Token[1] + " ', يجب أن تكون فقط مجال", o_tokens);
	}
}
