...
void parser_TwoPointOperator(string Token[2048], CLASS_TOKEN *o_tokens){
		...
		)
	{
		...
		if (Token[4] == "(")
		{
			...
			else if (L_FUN_IS_SET[std::make_pair(Token[1], Token[3])])
			{
				...
				ScriptSyntaxBuffer = CHECK_CALL_FUN_ARG(false, TheNamespace, Token[1], 0, TheNamespace, TheFunction, TempToken, 
		...
		else if (Token[4] == "=")
		{
			...
			*/
			...
			{
				ErrorCode("المجال ' " + Token[1] + " ' لا تحتوي على خاصيه باسم ' " + Token[3] + " ' ", o_tokens);
			}
		...
		else if (Token[4] == ":")
		{
			...
			// 	ErrorCode("المجال ' " + Token[1] + " ' لا تحتوي على اداه باسم ' " + Token[3] + " ' ", o_tokens);
	...
	// else if (CONTROL_IS_SET[std::make_pair(TheNamespace, Token[1])])
	// {
	// 	// ---------------------------------------------------------------------------------
	// 	// Control : Options
	// 	// ---------------------------------------------------------------------------------
