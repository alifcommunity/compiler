...
// Log **********************************************************
	...
	void clean_str_json(std::string &s){
		...
		if(ERROR_AS_JSON64){
			...
			//boost::beast::detail::base64::encode((void *)s.c_str(), (void *)s.c_str(), s.length());
			//size_t len = 0;
			//_base64_encode((const unsigned char *) s.c_str(), s.length(), &len);
			s = base64_encode(s);
	...
	void ALIF_ERROR(std::string ERROR_DESCRIPTION) 
	{
		...
		if(ERROR_AS_JSON){
			...
			ERROR_DESCRIPTION = "{\"file\":\"\",\"line\":\"0\",\"msg\":\"" + ERROR_DESCRIPTION + "\"}";
		}
...
// Tools ********************************************************
	...
	std::string base64_encode(std::string data){
		...
		pbuf = _base64_encode((const unsigned char *) data.c_str(), data.length(), &len);
		buf = reinterpret_cast<char*>(pbuf);
		//alifcore_trim(buf);

		//alifcore_free(pbuf);
		
		//std::cout << "Encode: [" << data << "] -> [" << buf << "] (" << len << " Bytes) \n";
	...
	std::string base64_decode(std::string data){
		...
		pbuf = _base64_decode((const unsigned char *) data.c_str(), data.length(), &len);
		buf = reinterpret_cast<char*>(pbuf);
		//alifcore_trim(buf);

		//alifcore_free(pbuf);
		
		//std::cout << "Decode: [" << data << "] -> [" << buf << "] (" << len << " Bytes) \n";
...
// Main *********************************************************
	...
	void alif(){
		...
		if(argument.log.fullpath == ""){
			...
			if(ERROR_AS_JSON)
				set_log_file(argument.input.path + argument.input.filename + argument.input.extension + ".json");
			else
	...
	#endif
	{
		...
		try {
			...
			desc.add_options()
				...
				("log-json-base64", boost::program_options::value<std::string>(), "Like 'log-json' but data is base64 encoded")
