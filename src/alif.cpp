...
#define ALIF_VERSION "4.0.0 (Beta)"
...
std::string CG_APP_DESTROY() {
    ...
    BUFFER =
		...
		//#ifdef __APPLE__
			//if (WINDOW_IS_CONSTRUCTION_)" +
        ID["رئيسية"] + R"() OBJ_CLASS_WINDOW_)" + ID["رئيسية"] +
        R"(->EndModal(0);
		//#else
			if (WINDOW_IS_CONSTRUCTION_)" +
        ID["رئيسية"] + R"() OBJ_CLASS_WINDOW_)" + ID["رئيسية"] +
...
std::string CG_WINX_CODE() {
	...
	NS_)" +
		...
		wxPoint()" +
            ...
            R"(, )" +
            CPP_WINDOW[std::make_pair(Namespace_Total_Names[i], "ارتوب")] +
            R"(),
		wxSize()" +
            CPP_WINDOW[std::make_pair(Namespace_Total_Names[i], "عرض")] +
            R"(, )" +
            CPP_WINDOW[std::make_pair(Namespace_Total_Names[i], "ارتفاع")] +
            R"(), )" +
            CPP_WINDOW[std::make_pair(Namespace_Total_Names[i], "شكل")] +
            R"( ) 
