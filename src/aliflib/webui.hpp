...
namespace webui
{
  ...
  public:
    ...
    unsigned short bind(std::string key_id,
                        void (*function_ref)(webui::event e)) const;
  ...
  public:
    ...
    ~window() { o_win.~_window(); }

    // -- Window -----------------------------

    bool is_show() const { return o_win.window_is_running(); }

    bool any_is_show() const { return o_win.any_window_is_running(); }
    ...
    unsigned short bind(const std::string &id,
                        void (*func_ref)(webui::event e)) const
    {

      return o_win.bind(id, func_ref);
    }

    unsigned short get_window_id() const { return o_win.get_window_number(); }

    void bind_all(void (*func_ref)(webui::event e)) { o_win.bind_all(func_ref); }

    void serve_folder(std::string p) { o_win.set_root_folder(p); }

    void serve_folder() { o_win.set_root_folder(""); }
    ...
    void allow_multi_serving(bool b) { o_win.allow_multi_access(b); }
    ...
    std::string new_server() { return o_win.new_server(nullptr); }

    std::string get_current_path() { return webui::get_current_path(); }
