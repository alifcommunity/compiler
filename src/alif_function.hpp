...
void parser_Function(std::string Token[2048], CLASS_TOKEN *o_tokens){
	...
	if (Token[FUN_POS + 1] == "رئيسية") // دالة رئيسية
	{
		if (IsInsideNamespace)
			ErrorCode("لا يمكن انشاء دالة رئيسية داخل مجال، بل يجب انشائها في المجال العام أو داخل صنف", o_tokens);
		...
		if (!o_tokens->TOKENS_PREDEFINED)
		{
			if (MAIN_FUN_IS_SET[TheNamespace])
				ErrorCode("الدالة الرئيسية تم انشاؤها مسبقا في السطر : " + MAIN_FUN_AT_LINE[TheNamespace], o_tokens);
		...
		if (Token[FUN_POS + 3] == "") // دالة TYPE MyFunctionName
		{
			if (Token[FUN_POS + 1] == "عدد")
			{
				...
				else
				{
					...
					{
						// Local function
						ADD_FUN(false, TheNamespace, Token[FUN_POS + 2], "عدد", o_tokens->Line, o_tokens);
			...
			else if (Token[FUN_POS + 1] == "نص")
			{
				...
				else
				{
					...
					{
						// Local function
						ADD_FUN(false, TheNamespace, Token[FUN_POS + 2], "نص", o_tokens->Line, o_tokens);
			...
			else if (Token[FUN_POS + 1] == "منطق")
			{
				...
				else
				{
					...
					{
						// Local function
						ADD_FUN(false, TheNamespace, Token[FUN_POS + 2], "منطق", o_tokens->Line, o_tokens);
	...
	else // void : دالة MyFunctionName (???) // void
	{
		...
		if (Token[FUN_POS + 2] == "") // دالة MyFunctionName
		{
			...
			else
			{
				...
				{
					// Local function
					ADD_FUN(false, TheNamespace, Token[FUN_POS + 1], "عادم", o_tokens->Line, o_tokens);
