...
void HTML_to_c(std::string sHTMLPath, std::string sCPath, std::string VarName,
  ...
  std::string cppCode_start =
      "static const std::string " + VarName + " = ( R\"V0G0N( \n\n";
  ...
  std::string cppCodeWebUI = R"( 
	const std::string )" +
                             VarName + R"(_webui_html = R"V0G0N(
	)" + sHTML.str() + R"(
	)V0G0N";
	webui::window )" + VarName +
                             R"(_webui_o; // &)" + VarName + R"(_webui_html
		)";
  ...
  CPP_WINDOW[std::make_pair(TheNamespace, "LOAD")] =
      CBUFER + " \n " + VarName + "_webui_o.show( &" + VarName +
      "_webui_html ); ";
}
  ...
  if (Token[3] == "رئيسية") {
    ...
    */
    ...
    CPP_WINDOW[std::make_pair("رئيسية", "CTR_CONSTRUCTOR")] =
        ...
        "(\"Alif_Application\")); o_TopSizer->Add(" +
        ...
        ".htm\"); \n Center(); \n Bind(wxEVT_WEBVIEW_NEWWINDOW, &NS_" +
        ID["رئيسية"] + "::AlifJavaScript_OnNewWindow, this, " + CBUFER_OBJ +
        "->GetId()); \n Bind(wxEVT_WEBVIEW_NAVIGATING, &NS_" + ID["رئيسية"] +
        "::AlifJavaScript_OnNavigationRequest, this, " + CBUFER_OBJ +
    ...
    CPP_WINDOW[std::make_pair(TheNamespace, "FUN_DECLARATION")] =
        CBUFER + " \n void AlifJavaScript_OnNewWindow(wxWebViewEvent& evt); \n "
                 ...
                 "evt); \n void AlifJavaScript_Error(std::string sError); void "
                 "AlifJavaScript_Run(std::string JSCode); \n void "
                 "AlifJavaScriptBridge(std::string sLink); \n ";
    ...
    // y = ارتوب
    /*
    CPP_WINDOW[std::make_pair(TheNamespace, "نص")] = " (" + TheNamespace + ")
    مجال بدون عنوان"; #ifdef __APPLE__ CPP_WINDOW[std::make_pair(TheNamespace,
    "افصول")] 	= "10"; CPP_WINDOW[std::make_pair(TheNamespace, "ارتوب")] =
    "25"; // Because the macOS task-bar is on top screen. #else
    ...
    Namespace_Total_Names[Namespace_Total] = TheNamespace; // only on second
    windows, no main. CPP_ID_DECLARATION.append(" int ID_WINDOW_" +
    ID[TheNamespace] + " = ALIFCORE_ID_GENERATOR(); \n"); CPP_GLOBAL.append("
    bool WINDOW_IS_SHOW_" + ID[TheNamespace] + " = false; \n");
    CPP_GLOBAL.append(" bool WINDOW_IS_CONSTRUCTION_" + ID[TheNamespace] + " =
    ...
    */
    ...
    if (DEBUG)
      DEBUG_MESSAGE("[NAMESPACE] [" + Token[3] + "] {SET BASE CTR} \n\n",
                    o_tokens); // DEBUG
    ...
    CPP_WINDOW[std::make_pair(Token[3], "CTR_CONSTRUCTOR")] =
        ...
        "->GetId()); \n Bind(wxEVT_WEBVIEW_NAVIGATING, &NS_" + ID[Token[3]] +
        "::AlifJavaScript_OnNavigationRequest, this, " + CBUFER_OBJ +
        "->GetId()); \n " + CBUFER_OBJ + "->EnableContextMenu(false); \n " +
        CBUFER_OBJ + "->EnableHistory(false); \n ";
    ...
    CPP_WINDOW[std::make_pair(TheNamespace, "نص")] =
        " (" + TheNamespace + ") مجال بدون عنوان";
#ifdef __APPLE__
    CPP_WINDOW[std::make_pair(TheNamespace, "افصول")] = "10";
    CPP_WINDOW[std::make_pair(TheNamespace, "ارتوب")] =
    ...
    CPP_WINDOW[std::make_pair(TheNamespace, "ارتفاع")] = "400";
    Namespace_Total++; // only on second windows, no main.
    Namespace_Total_Names[Namespace_Total] =
        TheNamespace; // only on second windows, no main.
    CPP_ID_DECLARATION.append(" int ID_WINDOW_" + ID[TheNamespace] +
                              " = ALIFCORE_ID_GENERATOR(); \n");
    CPP_GLOBAL.append(" bool WINDOW_IS_SHOW_" + ID[TheNamespace] +
                      " = false; \n");
    CPP_GLOBAL.append(" bool WINDOW_IS_CONSTRUCTION_" + ID[TheNamespace] +
                      " = false; \n");
    CPP_GLOBAL.append(" void WINDOW_LOAD_" + ID[TheNamespace] + "(); \n");
