...
void parser_Class(std::string Token[2048], CLASS_TOKEN *o_tokens)
{
  ...
  if (IsInsideFunction)
    ErrorCode("لا يمكن أنشاء صنف داخل دالة، يجب إنشائها في المجال عام فقط",
              o_tokens);

  if (IsInsideNamespace)
    ErrorCode("لا يمكن أنشاء صنف داخل مجال، يجب إنشائها في المجال عام فقط",
              o_tokens);
  ...
  // TheNamespace = TheClass; // < ======================================= TODO:
  // TO BE CHECK IF THIS WORKAROUND WORK FINE !!!!!!
