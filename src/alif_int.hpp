...
void parser_Int(std::string Token[2048], CLASS_TOKEN *o_tokens) {
  ...
  if (IsInsideNamespace && !IsInsideFunction)
    ErrorCode("انشاء متغير جديد يجب ان يكون خارج المجال ليكون عام، او داخل "
              "دالة ليكون محلي",
              o_tokens);
  ...
  if (IsInsideClass) {
    if (!IsInsideFunction) {
      ...
      if (Token[1] == "خاص") {
        ...
        if (DEBUG)
          DEBUG_MESSAGE("[PRIVATE] [CLASS-GLOBAL-INT] [" + Token[INT_POS + 1] +
                            "] ",
                        o_tokens); // DEBUG
  ...
  // Global INT
    ...
    SetNewVar(false, TheNamespace, TheFunction, Token[INT_POS + 1], "عدد",
