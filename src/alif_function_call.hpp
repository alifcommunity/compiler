...
void parser_FunctionCall(std::string Token[2048], CLASS_TOKEN *o_tokens) {
  ...
  لذلك أنت بحاجة إلى متغير للحصول على قيمة الرجوع", o_tokens);
          ...
          if(DEBUG)DEBUG_MESSAGE("[CALL-LOCAL-FUNCTION ' " + Token[1] + " '] (",
  o_tokens); // DEBUG

          // *** Generate Code ***
          // Call Local Func from inside Local Func
          cpp_AddScript(TheFunction, " NS_" + ID[TheNamespace] + "::FUNCTION_" +
  ID[Token[1]] + " (");
          // *** *** *** *** *** ***

          // Check local fun Args and Syntax : fun (a + c, 2 * (b - 1))
          ScriptSyntaxBuffer = CHECK_CALL_FUN_ARG(false, TheNamespace, Token[1],
  0, TheNamespace, TheFunction, TempToken, (TempTokenCount - 1),  o_tokens);
  ...
  لذلك أنت بحاجة إلى متغير للحصول على قيمة الرجوع", o_tokens);
          ...
          if (IsInsideClass)
          {
                  if(DEBUG)DEBUG_MESSAGE("[CALL-GLOBAL-FUNCTION ' " + Token[1] +
  " '] ( ", o_tokens); // DEBUG
