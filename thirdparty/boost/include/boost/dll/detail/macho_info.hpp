...
private:
    ...
    template <class F>
    static void command_finder(boost::filesystem::ifstream& fs, uint32_t cmd_num, F callback_f) {
        ...
        for (std::size_t i = 0; i < h.ncmds; ++i) {
            ...
            if (command.cmd != cmd_num) {
                fs.seekg(pos + static_cast<boost::filesystem::ifstream::pos_type>(command.cmdsize));
                continue;
            }
    ...
    struct section_names_gather {
        ...
        void operator()(boost::filesystem::ifstream& fs) const {
            segment_t segment;
            read_raw(fs, segment);
