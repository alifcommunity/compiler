...
public:
    static bool parsing_supported(boost::filesystem::ifstream& fs) {
    ...
    static std::vector<std::string> sections(boost::filesystem::ifstream& fs) {
...
private:
    ...
    static void checked_seekg(boost::filesystem::ifstream& fs, Integer pos) {
    ...
    template <class T>
    static void read_raw(boost::filesystem::ifstream& fs, T& value, std::size_t size = sizeof(T)) {
        fs.read(reinterpret_cast<char*>(&value), size);
    }

    static header_t header(boost::filesystem::ifstream& fs) {
        header_t elf;
    ...
    static void sections_names_raw(boost::filesystem::ifstream& fs, std::vector<char>& sections) {
    ...
    static void symbols_text(boost::filesystem::ifstream& fs, std::vector<symbol_t>& symbols, std::vector<char>& text) {
        const header_t elf = header(fs);
        ...
        for (std::size_t i = 0; i < elf.e_shnum; ++i) {
            ...
            if (section.sh_type == SHT_SYMTAB_) {
                ...
                const boost::filesystem::ifstream::pos_type pos = fs.tellg();
...
public:
    static std::vector<std::string> symbols(boost::filesystem::ifstream& fs) {
        std::vector<std::string> ret;
    ...
    static std::vector<std::string> symbols(boost::filesystem::ifstream& fs, const char* section_name) {
        std::vector<std::string> ret;
